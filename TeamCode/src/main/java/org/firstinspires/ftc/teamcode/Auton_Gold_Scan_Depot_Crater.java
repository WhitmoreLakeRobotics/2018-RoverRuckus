...
@Autonomous(name = "Auton_Scan_Depot_Crater ", group = "Auton")
// @Autonomous(...) is the other common choice

public class Auton_Gold_Scan_Depot_Crater extends OpMode {
    ...
    private static final int stage31_scan = 31;

    private static final int stage33_centerMine = 33;
    private static final int stage34_centerMine = 34;

    private static final int stage37_sideMine = 37;

    private static final int stage39_2Depot = 39;
    private static final int stage40_empty = 40;
    private static final int stage45_empty = 45;
...
// check stage and do what's appropriate
        ...
        if (currentStage == stage30_drive) {
            if (RBTChassis.hanger.isExtended()) {
                ...
                RBTChassis.scannerArms.cmdMoveDown();
                currentStage = stage31_scan;
            }
        }

        if (currentStage == stage31_scan) {
            if (RBTChassis.getcmdComplete()){
                if (RBTChassis.scannerArms.isGoldCenter()) {
                    //Move both arms up...
                    RBTChassis.scannerArms.cmdMoveUp();
                    RBTChassis.intakeArm.cmd_moveToPickupPos();
                    currentStage = stage33_centerMine;
                }
                else if (RBTChassis.scannerArms.isGoldLeft()) {
                    RBTChassis.scannerArms.cmdMoveUpRight();
                    currentStage = stage37_sideMine;
                }
                else if (RBTChassis.scannerArms.isGoldRight()) {
        ...
        if (currentStage == stage33_centerMine){
            if (RBTChassis.scannerArms.getIsUp() &&
                    RBTChassis.intakeArm.atDestination(IntakeArm.IntakeDestinations.IntakeDestinations_Pickup)) {
                RBTChassis.cmdDrive(-AUTO_DRIVEPower / 2, 0, -4);
                currentStage = stage39_2Depot;
            }
        }

        if (currentStage == stage34_centerMine){
            if (RBTChassis.scannerArms.getIsUp() &&
                    RBTChassis.intakeArm.atDestination(IntakeArm.IntakeDestinations.IntakeDestinations_Pickup)) {
                RBTChassis.cmdDrive(AUTO_DRIVEPower / 2, 0, 8);
                currentStage = stage39_2Depot;
            }
        }
        if (currentStage == stage37_sideMine){
            if (RBTChassis.scannerArms.getIsUpLeft() || RBTChassis.scannerArms.getIsUpRight()) {
                RBTChassis.cmdDrive(AUTO_DRIVEPower / 2, 0, 4);
                currentStage = stage39_2Depot;
            }
        }

        if (currentStage == stage39_2Depot) {
            if (RBTChassis.getcmdComplete()) {
        ...
        if (currentStage == stage40_empty) {
            if (RBTChassis.getcmdComplete()) {
                ...
                RBTChassis.intakeArm.cmd_moveToPickupPos();
                currentStage = stage45_empty;
            }
        }

        if (currentStage == stage45_empty) {
            if (RBTChassis.intakeArm.atDestination(IntakeArm.IntakeDestinations.IntakeDestinations_Pickup) == true) {
